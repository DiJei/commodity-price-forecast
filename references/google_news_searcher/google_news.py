...
class GoogleNews:
    ...
    def __init__(self,proxy_enable=False,proxy_host=[],lang="en",period="",start="",end="",encode="utf-8",region=None):
        self.__texts = []
        self.__links = []
        self.__results = []
        self.__totalcount = 0
    ...
    def build_response(self):
        if self.proxy_enable:
            proxy_handler = urllib.request.ProxyHandler({'http':self.proxy_host[0],'https':self.proxy_host[1]})
            opener = urllib.request.build_opener(proxy_handler)
            opener.addheaders = [('User-agent', self.user_agent)]
            req = opener.open(self.url.replace("search?","search?hl=en&gl=en&"))
            self.page = req.read
        else:
